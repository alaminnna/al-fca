...
function buildParticipantListHash(participants) {
  ...
  const hash = (0, import_node_crypto.createHash)("sha256").update(sorted.join("")).digest();
...
async function sendE2EETextAction(ctx, threadId, text, replyToMessageId, replyToSenderJid) {
  ...
  const msgNode = encodeNode("message", { to: toJid, type: "text", id: messageId }, [
    ...
    encodeNode("trace", {}, [
      encodeNode("request_id", {}, Buffer.from((0, import_node_crypto2.randomUUID)().replace(/-/g, ""), "hex"))
    ])
  ]);
...
async function sendE2EEGroupTextAction(ctx, groupJid, text, replyToMessageId, replyToSenderJid) {
  ...
  const traceNode = encodeNode("trace", {}, [
    encodeNode("request_id", {}, Buffer.from((0, import_node_crypto3.randomUUID)().replace(/-/g, ""), "hex"))
  ]);
...
async function sendE2EERevokeAction(ctx, threadId, messageId, fromMe) {
  ...
  if (isGroup) {
    ...
    const msgNode = encodeNode("message", { to: threadId, type: "text", id: newMessageId, phash, edit: editAttr }, [
      ...
      encodeNode("trace", {}, [encodeNode("request_id", {}, Buffer.from((0, import_node_crypto4.randomUUID)().replace(/-/g, ""), "hex"))]),
...
async function sendE2EEReactionAction(ctx, threadId, messageId, emoji, senderJid) {
  ...
  if (isGroup) {
    ...
    const msgNode = encodeNode("message", { to: threadId, type: "text", id: newMessageId, phash }, [
      encodeNode("participants", {}, participantNodes),
      encodeNode("franking", {}, [encodeNode("franking_tag", {}, frankingTag)]),
      encodeNode("trace", {}, [encodeNode("request_id", {}, Buffer.from((0, import_node_crypto5.randomUUID)().replace(/-/g, ""), "hex"))]),
      encodeNode("enc", { v: "3", type: "skmsg", "decrypt-fail": "hide" }, groupResult.groupCiphertext)
    ]);
    await e2eeSocket.sendFrame(marshal(msgNode));
...
async function sendE2EEEdit(ctx, threadId, messageId, newText) {
  ...
  if (isGroup) {
    ...
    const msgNode = encodeNode("message", { to: threadId, type: "text", id: newMessageId, phash, edit: editAttr }, [
      encodeNode("participants", {}, participantNodes),
      encodeNode("franking", {}, [encodeNode("franking_tag", {}, frankingTag)]),
      encodeNode("trace", {}, [encodeNode("request_id", {}, Buffer.from((0, import_node_crypto6.randomUUID)().replace(/-/g, ""), "hex"))]),
      encodeNode("enc", { v: "3", type: "skmsg", "decrypt-fail": "hide" }, groupResult.groupCiphertext)
    ]);
    await e2eeSocket.sendFrame(marshal(msgNode));
...
async function sendE2EEMediaDM(ctx, input, mediaType, buildMessage) {
  ...
  const msgNode = encodeNode("message", { to: toJid, type: "text", id: messageId }, [
    ...
    encodeNode("trace", {}, [
      encodeNode("request_id", {}, import_node_buffer.Buffer.from((0, import_node_crypto7.randomUUID)().replace(/-/g, ""), "hex"))
    ])
  ]);
  await e2eeSocket.sendFrame(marshal(msgNode));
  ctx.outgoingE2EECache.remember({
...
function encodeMessageApplication(consumerAppBytes, replyTo) {
  const frankingKey = (0, import_node_crypto9.randomBytes)(32);
  ...
  const frankingTag = (0, import_node_crypto9.createHmac)("sha256", frankingKey).update(messageApp).digest();
...
function generatePadding() {
  const len = (0, import_node_crypto10.randomBytes)(1)[0] & 255 || 1;
  const pad = (0, import_node_crypto10.randomBytes)(len);
  pad[len - 1] = len;
  return pad;
}
...
var DeviceStore = class _DeviceStore {
  ...
  async initNew() {
    ...
    this.noiseKeyPriv = (0, import_node_crypto12.randomBytes)(32);
    ...
    const buf = (0, import_node_crypto12.randomBytes)(2);
    this.registrationId = buf.readUInt16BE(0) & 16383 || 1;
    this.advSecretKey = (0, import_node_crypto12.randomBytes)(32);
    this.facebookUUID = (0, import_node_crypto13.randomUUID)();
...
function expandMediaKey(mediaKey, type) {
  ...
  const expanded = Buffer.from(
    (0, import_node_crypto14.hkdfSync)("sha256", mediaKey, Buffer.alloc(0), info, 112)
  );
  return {
...
function encryptMedia(plaintext, type) {
  const mediaKey = (0, import_node_crypto14.randomBytes)(32);
  const { iv, cipherKey, macKey } = expandMediaKey(mediaKey, type);
  const cipher = (0, import_node_crypto14.createCipheriv)("aes-256-cbc", cipherKey, iv);
  const ciphertext = Buffer.concat([cipher.update(plaintext), cipher.final()]);
  const mac = (0, import_node_crypto14.createHmac)("sha256", macKey).update(iv).update(ciphertext).digest().subarray(0, 10);
  const dataToUpload = Buffer.concat([ciphertext, mac]);
  return {
    mediaKey,
    fileSHA256: Buffer.from((0, import_node_crypto14.createHash)("sha256").update(plaintext).digest()),
    fileEncSHA256: Buffer.from((0, import_node_crypto14.createHash)("sha256").update(dataToUpload).digest()),
...
function decryptMedia(opts) {
  ...
  if (expectedFileEncSHA256) {
    const actual = (0, import_node_crypto14.createHash)("sha256").update(data).digest();
  ...
  const { iv, cipherKey, macKey } = expandMediaKey(mediaKey, type);
  const expectedMac = (0, import_node_crypto14.createHmac)("sha256", macKey).update(iv).update(ciphertext).digest().subarray(0, MEDIA_MAC_LENGTH);
  if (!expectedMac.equals(mac)) {
    throw new Error("Invalid media HMAC - data corrupted or wrong key");
  }
  const decipher = (0, import_node_crypto14.createDecipheriv)("aes-256-cbc", cipherKey, iv);
  const plaintext = Buffer.concat([decipher.update(ciphertext), decipher.final()]);
  if (expectedFileSHA256) {
    const actual = (0, import_node_crypto14.createHash)("sha256").update(plaintext).digest();
...
async function createSenderKeyDistributionMessage(store, groupJid, senderJid) {
  const distributionId = (0, import_node_crypto15.randomUUID)();
...
async function encryptGroup(store, groupJid, senderJid, plaintext, distributionId) {
  const activeDistributionId = distributionId ?? (0, import_node_crypto15.randomUUID)();
  const senderAddr = jidToAddress(senderJid);
...
function generateX25519() {
  const { privateKey, publicKey } = (0, import_node_crypto16.generateKeyPairSync)("x25519");
...
function getX25519PublicKey(privKeyRaw) {
  const priv = (0, import_node_crypto16.createPrivateKey)({ key: Buffer.concat([PKCS8_HEADER, privKeyRaw]), format: "der", type: "pkcs8" });
  const pubDer = (0, import_node_crypto16.createPublicKey)(priv).export({ type: "spki", format: "der" });
  return Buffer.from(pubDer.subarray(pubDer.length - 32));
}
function x25519DH(privKeyRaw, pubKeyRaw) {
  const pub = (0, import_node_crypto16.createPublicKey)({ key: Buffer.concat([SPKI_HEADER, pubKeyRaw]), format: "der", type: "spki" });
  const priv = (0, import_node_crypto16.createPrivateKey)({ key: Buffer.concat([PKCS8_HEADER, privKeyRaw]), format: "der", type: "pkcs8" });
  return (0, import_node_crypto16.diffieHellman)({ privateKey: priv, publicKey: pub });
}
...
var NoiseHandshakeState = class {
  ...
  sha256(data) {
    return Buffer.from((0, import_node_crypto16.createHash)("sha256").update(data).digest());
  }
  mixHash(data) {
    this.h = this.sha256(Buffer.concat([this.h, data]));
  }
  mixKey(input) {
    const expanded = Buffer.from((0, import_node_crypto16.hkdfSync)("sha256", input, this.ck, "", 64));
  ...
  encrypt(plaintext) {
    ...
    const cipher = (0, import_node_crypto16.createCipheriv)("aes-256-gcm", this.k, nonce);
  ...
  decrypt(ciphertext) {
    ...
    try {
      const decipher = (0, import_node_crypto16.createDecipheriv)("aes-256-gcm", this.k, nonce);
  ...
  finish() {
    const expanded = Buffer.from((0, import_node_crypto16.hkdfSync)("sha256", Buffer.alloc(0), this.ck, Buffer.alloc(0), 64));
    return {
...
var EncryptedFrameSocket = class {
  ...
  encryptFrame(plaintext) {
    ...
    const cipher = (0, import_node_crypto16.createCipheriv)("aes-256-gcm", this.sendKey, nonce);
    const enc = Buffer.concat([cipher.update(plaintext), cipher.final(), cipher.getAuthTag()]);
  ...
  decryptFrame(data) {
    ...
    const decipher = (0, import_node_crypto16.createDecipheriv)("aes-256-gcm", this.recvKey, nonce);
    decipher.setAuthTag(tag);
