...
module.exports = function (defaultFuncs, api, ctx) {
  return async function markAsRead(threadID, read, callback) {
    ...
    // E2EE JIDs carry the numeric thread as prefix; read receipts are thread
    // metadata and go over the normal channel like typing indicators.
    if (typeof threadID === "string" && /@msgr$/i.test(threadID)) {
        var _m = threadID.match(/^(\d+)/);
        if (_m) threadID = _m[1];
    }
