...
function buildAPI(globalOptions, html, jar) {
    ...
    // Invariant: every directly-nested src/*.js file must export a factory
    // function (defaults, api, ctx); the loader calls it to build api[name].
    // Non-factory helpers are excluded from the filter below.
    require('fs').readdirSync(__dirname + '/src/').filter(v => v.endsWith('.js') && v !== 'logger.js').forEach(v => { api[v.replace('.js', '')] = require(`./src/${v}`)(utils.makeDefaults(html, userID, ctx), api, ctx); });
